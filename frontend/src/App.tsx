...
export default function App() {
  ...
  function 输入框按键(e: React.KeyboardEvent<HTMLTextAreaElement>) {
  ...
  return (
    <main style={{ display: 'grid', gridTemplateColumns: '280px 1fr 340px', gap: 14, fontFamily: 'Segoe UI, PingFang SC', padding: 16 }}>
      ...
      <section style={{ border: '1px solid #ddd', borderRadius: 10, padding: 12 }}>
        <h2 style={{ marginTop: 0 }}>中文本地 AI 工作台（第三阶段 B 轻量体验补丁）</h2>
        ...
        />
        <button onClick={发送} disabled={发送中}>
          {发送中 ? '发送中...' : '发送（非流式）'}
        </button>
      ...
      <aside style={{ border: '1px solid #ddd', borderRadius: 10, padding: 12 }}>
        ...
        <ul style={{ paddingLeft: 16 }}>
          {文档列表.map((d) => (
            <li key={d.id}>
              {d.file_name}
              <button onClick={() => 删除某个文档(d.id)} style={{ marginLeft: 6 }}>删除</button>
            </li>
          ))}
        </ul>
        <h4>检索片段</h4>
        <input value={检索词} onChange={(e) => set检索词(e.target.value)} placeholder="输入关键词" />
        <button onClick={查询片段} disabled={!当前工作区 || 检索中}>{检索中 ? '检索中...' : '检索'}</button>
